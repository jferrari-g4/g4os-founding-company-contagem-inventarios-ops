...
export default function PlanningEditor({projects,initialProjectId='',user,edit,canDimension=false,canApproveTi=false,canApprove=false,catalogs={},onDirtyChange,onSaved}){
  ...
  const planningAvailable=Boolean(project&&['planning','awaiting_dates','confirmed'].includes(project.status));
  const isReadOnly=project?.status==='confirmed';
  const audit=useMemo(()=>auditPlanning(content),[content]);
  const currentSnapshot=useMemo(()=>JSON.stringify(content),[content]);
  const isDirty=Boolean(project&&savedSnapshot&&currentSnapshot!==savedSnapshot);
  const selectProject=id=>{if(id===selected)return true;if(isDirty&&!window.confirm('Existem alterações não salvas. Deseja trocar de operação e descartá-las?'))return false;setSelected(id);requestAnimationFrame(()=>document.getElementById('planning-editor')?.scrollIntoView({behavior:'smooth',block:'start'}));return true};
  const scrollToAuditBlock=key=>{const target=document.getElementById(key==='equipment'?'clientEquipment':key);if(!target)return;target.scrollIntoView({behavior:'smooth',block:'start'});window.setTimeout(()=>target.querySelector('input,textarea,select,button')?.focus({preventScroll:true}),300)};
  const auditKey=key=>['clientEquipment','contagemEquipment'].includes(key)?'equipment':key;
  const blockAudit=key=>key==='checklists'?audit.blocks.filter(block=>block.key.startsWith('checklist-')).flatMap(block=>block.issues):audit.blocks.find(block=>block.key===auditKey(key))?.issues||[];
  const isBlockDone=key=>Boolean(content.completedBlocks?.[key])&&blockAudit(key).length===0;
  const toggleBlock=key=>{if(isReadOnly)return;const done=isBlockDone(key);if(!done){const issues=blockAudit(key);if(issues.length){setNotice(`Resolva ${issues.length} pendência(s) antes de concluir este bloco.`);return}}set(['completedBlocks',key],!done)};
  const reopenChangedBlock=(next,key)=>{const blockKey=['clientEquipment','contagemEquipment'].includes(key)?key:key==='checklists'?'checklists':key;if(next.completedBlocks?.[blockKey])next.completedBlocks[blockKey]=false};

  useEffect(()=>{if(!project){setContent(initial());setSavedSnapshot('');return}const saved=project.planning?.content;const next=merge(saved);if(!saved&&Array.isArray(catalogs.checklists)){const templateFor=type=>catalogs.checklists.find(template=>template.checklist_type===type)?.checklist_template_items?.sort((a,b)=>(a.sort_order||0)-(b.sort_order||0)).map(item=>({id:uid(),label:item.label,done:false,required:item.required!==false}))||null;const pre=templateFor('pre_inventory_visit');const planning=templateFor('planning');if(pre?.length)next.checklists.preInventory=pre;if(planning?.length)next.checklists.planning=planning}setContent(next);setSavedSnapshot(JSON.stringify(next));setNotice('')},[project?.id,project?.planning?.updated_at,project?.planning?.version]);
  useEffect(()=>{if(initialProjectId&&initialProjectId!==selected&&projects.some(item=>item.id===initialProjectId)){if(isDirty&&!window.confirm('Existem alterações não salvas. Deseja abrir outra operação e descartá-las?'))return;setSelected(initialProjectId);requestAnimationFrame(()=>document.getElementById('planning-editor')?.scrollIntoView({behavior:'smooth',block:'start'}))}},[initialProjectId]);
  useEffect(()=>{if(!isDirty)return;const warn=event=>{event.preventDefault();event.returnValue=''};window.addEventListener('beforeunload',warn);return()=>window.removeEventListener('beforeunload',warn)},[isDirty]);
  useEffect(()=>{onDirtyChange?.(isDirty)},[isDirty,onDirtyChange]);
  useEffect(()=>()=>onDirtyChange?.(false),[onDirtyChange]);

  const set=(path,value)=>setContent(current=>{const next=structuredClone(current);let target=next;path.slice(0,-1).forEach(key=>target=target[key]);target[path.at(-1)]=value;reopenChangedBlock(next,path[0]);return next});
  const add=(path,item)=>setContent(current=>{const next=structuredClone(current);let target=next;path.forEach(key=>target=target[key]);target.push(item);reopenChangedBlock(next,path[0]);return next});
  const remove=(path,index)=>setContent(current=>{const next=structuredClone(current);let target=next;path.forEach(key=>target=target[key]);target.splice(index,1);reopenChangedBlock(next,path[0]);return next});
  const row=(path,index,key,value)=>setContent(current=>{const next=structuredClone(current);let target=next;path.forEach(pathKey=>target=target[pathKey]);target[index][key]=value;reopenChangedBlock(next,path[0]);return next});
  ...
  async function save(approve=false){
    if(!project)return;
    if(!planningAvailable||isReadOnly){setNotice('Este planejamento está disponível somente para consulta nesta etapa.');return}
  ...
  return <section className="planning-shell">
    <PlanningKanban projects={projects} selectedId={selected} onSelect={selectProject} edit={edit} canDimension={canDimension} canApproveTi={canApproveTi}/>
    <div className="card planning-header" id="planning-editor">
      ...
      <div className="planning-tools"><select value={selected} onChange={event=>selectProject(event.target.value)}><option value="">Selecione uma solicitação</option>{projects.map(item=><option key={item.id} value={item.id}>{item.company_name} · {item.reference}</option>)}</select>{project&&<><span className={`save-state ${isReadOnly?'is-locked':isDirty?'is-dirty':'is-saved'}`}>{isReadOnly?'Somente consulta':isDirty?'Alterações não salvas':'Tudo salvo'}</span><div className="export-actions"><button type="button" onClick={exportPdf}>Exportar PDF</button><button type="button" onClick={exportExcel}>Exportar Excel</button></div></>}</div>
    </div>
    {!project?<div className="card"><p>Crie ou selecione uma solicitação para começar o planejamento.</p></div>:!planningAvailable?<div className="card stage-gate-card"><span className={`kanban-status kanban-status-${project.status}`}>{planningStatus[project.status]||project.status}</span><h2>Esta operação ainda não chegou ao preenchimento do planejamento</h2><p>Use as ações do cartão no Kanban para concluir o gate atual. Os blocos serão liberados após a validação necessária.</p><button type="button" onClick={()=>document.querySelector('.kanban-card.is-selected')?.scrollIntoView({behavior:'smooth',block:'center'})}>Voltar ao cartão</button></div>:<>
      <div className={`card progress-card ${isReadOnly?'is-readonly':''}`}>
        <div className="progress-heading"><div><b>{project.company_name}</b><small>{project.reference} · {planningStatus[project.status]||project.status}</small></div><div className="progress-status"><span className={`save-state ${isReadOnly?'is-locked':isDirty?'is-dirty':'is-saved'}`}>{isReadOnly?'Planejamento bloqueado':isDirty?'Alterações não salvas':'Rascunho salvo'}</span><b>{audit.percentage}% concluído</b></div></div>{isReadOnly&&<div className="readonly-banner"><b>Planejamento concluído</b><span>Consulta liberada. Alterações exigem reabertura controlada e histórico.</span></div>}
      ...
      <Block readOnly={isReadOnly} blockKey="store" completed={isBlockDone('store')} onToggle={()=>toggleBlock('store')} title="1. Informações da loja" subtitle="Períodos e efetivo geral">
      ...
      <Block readOnly={isReadOnly} blockKey="responsibilities" completed={isBlockDone('responsibilities')} onToggle={()=>toggleBlock('responsibilities')} title="2. Responsáveis pela operação" subtitle="Funções configuráveis e responsáveis">
      ...
      <Block readOnly={isReadOnly} blockKey="logistics" completed={isBlockDone('logistics')} onToggle={()=>toggleBlock('logistics')} title="3. Logística" subtitle="Transporte, hospedagem e orientações">
        <h3>Transporte</h3>
      ...
      <Equipment readOnly={isReadOnly} blockKey="clientEquipment" completed={isBlockDone('clientEquipment')} onToggle={()=>toggleBlock('clientEquipment')} title="4. Equipamentos do cliente" value={content.clientEquipment} path="clientEquipment" setContent={setContent}/>
      <Equipment readOnly={isReadOnly} blockKey="contagemEquipment" completed={isBlockDone('contagemEquipment')} onToggle={()=>toggleBlock('contagemEquipment')} title="5. Equipamentos Contagem" value={content.contagemEquipment} path="contagemEquipment" setContent={setContent}/>
      <Block readOnly={isReadOnly} blockKey="preCount" completed={isBlockDone('preCount')} onToggle={()=>toggleBlock('preCount')} title="6. Pré-contagem" subtitle="Datas, horários, equipe e efetivo">
      ...
      <Block readOnly={isReadOnly} blockKey="days" completed={isBlockDone('days')} onToggle={()=>toggleBlock('days')} title="7. Dias, turnos e áreas do inventário" subtitle="Adicione quantos dias e sub-blocos forem necessários">
      ...
      <Block readOnly={isReadOnly} blockKey="checklists" completed={isBlockDone('checklists')} onToggle={()=>toggleBlock('checklists')} title="8. Checklists" subtitle="Edite os itens conforme a lista final da Contagem; os três checklists precisam estar preenchidos e concluídos para aprovação.">
      ...
      <div className={`sticky-actions ${isReadOnly?'is-readonly':''}`}><span>{isReadOnly?'Planejamento concluído e protegido contra edição.':notice||(isDirty?'Há alterações não salvas.':!canApprove?'A aprovação é restrita a administradores, supervisores e gestores operacionais.':!approvalStageReady?'Este projeto ainda não está em uma etapa elegível para aprovação.':'Rascunho salvo.')}</span>{!isReadOnly&&<><button onClick={()=>save(false)} disabled={saving||!isDirty}>{saving?'Salvando…':'Salvar rascunho'}</button><button className="primary" onClick={()=>save(true)} disabled={saving||!canApprove||!approvalStageReady} title={!canApprove?'Seu perfil não possui permissão para aprovar.':!approvalStageReady?'Projeto fora da etapa de aprovação.':''}>Aprovar planejamento</button></>}</div>
    </>}
  </section>
}

function PlanningKanban({projects,selectedId,onSelect,edit,canDimension=false,canApproveTi=false}){const [search,setSearch]=useState('');const [dates,setDates]=useState({});const [pendingAction,setPendingAction]=useState('');const normalizedSearch=search.trim().toLocaleLowerCase('pt-BR');const active=projects.filter(item=>item.status!=='cancelled');const filtered=normalizedSearch?active.filter(item=>[item.company_name,item.trade_name,item.cnpj,item.reference,item.inventory_type].filter(Boolean).some(value=>String(value).toLocaleLowerCase('pt-BR').includes(normalizedSearch))):active;const toLocal=value=>{if(!value)return '';const date=new Date(value);if(Number.isNaN(date.getTime()))return String(value).slice(0,16);const local=new Date(date.getTime()-date.getTimezoneOffset()*60000);return local.toISOString().slice(0,16)};const toIso=value=>{const date=new Date(value);return Number.isNaN(date.getTime())?value:date.toISOString()};function updateDate(id,key,value){setDates(current=>({...current,[id]:{...current[id],[key]:value}}))}async function saveVisitOptions(item){if(!canDimension)return;const values=dates[item.id]||{};const first=values.a||toLocal(item.date_options?.[0]);const second=values.b||toLocal(item.date_options?.[1]);if(!first||!second)return alert('Informe duas opções de data e horário para a visita de dimensionamento.');if(!window.confirm('Salvar estas duas opções de visita para esta operação?'))return;const actionKey=`${item.id}:dates`;setPendingAction(actionKey);try{await edit?.(item.id,{date_options:[toIso(first),toIso(second)]},'Duas opções de visita de dimensionamento registradas no Planejamento')}finally{setPendingAction('')}}async function action(item,patch,message,confirmation){if(confirmation&&!window.confirm(confirmation))return;const actionKey=`${item.id}:${patch.status||'update'}`;setPendingAction(actionKey);try{await edit?.(item.id,patch,message)}finally{setPendingAction('')}}return <section className="planning-kanban"><div className="kanban-heading"><div><span className="eyebrow">Fluxo único operacional</span><h2>Inventários em movimento</h2><p>Entrada, dimensionamento, TI, planejamento e conclusão no mesmo Kanban. Selecione um cartão para trabalhar na operação.</p><div className="kanban-search"><input aria-label="Pesquisar inventários" value={search} onChange={event=>setSearch(event.target.value)} placeholder="Pesquisar empresa, CNPJ ou referência"/>{search&&<button type="button" onClick={()=>setSearch('')}>Limpar</button>}</div></div><span className="kanban-total">{filtered.length} {filtered.length===1?'inventário':'inventários'}</span></div><div className="kanban-board">{planningColumns.map(column=>{const items=filtered.filter(item=>column.statuses.includes(item.status));return <div className={`kanban-column kanban-column-${column.key}`} key={column.key}><div className="kanban-column-head"><div><span className="kanban-dot"/><b>{column.label}</b></div><span>{items.length}</span></div><div className="kanban-cards">{items.length?items.map(item=>{const planningContent=item.planning?.content||{};const itemAudit=auditPlanning(planningContent);const store=planningContent.store||{};const datesLabel=store.inventoryStart&&store.inventoryEnd?`${formatDate(store.inventoryStart)} – ${formatDate(store.inventoryEnd)}`:'Período não definido';const pending=item.planning?itemAudit.approvalIssues.length:null;const optionState=dates[item.id]||{};const busy=pendingAction.startsWith(item.id);return <article className={`kanban-card ${selectedId===item.id?'is-selected':''}`} key={item.id} onClick={()=>onSelect(item.id)}><div className="kanban-card-top"><span className="kanban-reference">{item.reference||'Sem referência'}</span><span className={`kanban-status kanban-status-${item.status}`}>{planningStatus[item.status]||item.status}</span></div><div className="kanban-card-title-row"><h3>{item.company_name||'Cliente sem cadastro'}</h3><button type="button" className="kanban-card-select" aria-pressed={selectedId===item.id} aria-label={`Selecionar operação ${item.company_name}`} onClick={event=>{event.stopPropagation();onSelect(item.id)}}>Selecionar</button></div><p className="kanban-meta">{store.unit||item.inventory_type||'Unidade não definida'}</p><div className="kanban-card-info"><span><b>Período</b>{datesLabel}</span><span><b>Dias</b>{planningContent.days?.length||0}</span></div><div className="kanban-progress-line"><i style={{width:`${itemAudit.percentage}%`}}/></div><div className="kanban-card-bottom"><span>{item.planning?`${itemAudit.percentage}% preenchido`:'Planejamento será iniciado após o gate'}</span>{pending===null?<span className="kanban-pending neutral">Ação necessária</span>:pending>0?<span className="kanban-pending">{pending} pendência(s)</span>:<span className="kanban-pending ready">Pronto</span>}</div><div className="kanban-card-actions" onClick={event=>event.stopPropagation()}>{item.status==='received'&&<button type="button" disabled={!canDimension||busy} onClick={()=>action(item,{status:'awaiting_dimensioning'},'Dimensionamento iniciado no Planejamento','Iniciar o dimensionamento desta operação?')}>{busy?'Processando…':'Iniciar dimensionamento'}</button>}{item.status==='awaiting_dimensioning'&&<><div className="kanban-datebox"><input aria-label="Primeira opção de data da visita" type="datetime-local" value={optionState.a??toLocal(item.date_options?.[0])} onChange={event=>updateDate(item.id,'a',event.target.value)}/><input aria-label="Segunda opção de data da visita" type="datetime-local" value={optionState.b??toLocal(item.date_options?.[1])} onChange={event=>updateDate(item.id,'b',event.target.value)}/><button type="button" disabled={!canDimension||busy} onClick={()=>saveVisitOptions(item)}>{pendingAction===`${item.id}:dates`?'Salvando…':'Salvar opções da visita'}</button></div><button type="button" disabled={!canDimension||busy} onClick={()=>action(item,{status:'awaiting_ti'},'Dimensionamento concluído; encaminhado para TI','Concluir o dimensionamento e encaminhar esta operação para TI?')}>{busy?'Processando…':'Concluir dimensionamento'}</button></>}{item.status==='awaiting_ti'&&<button type="button" disabled={!canApproveTi||busy} onClick={()=>action(item,{status:'planning'},'TI aprovou layout e sistema; planejamento liberado','Confirmar a validação de TI e liberar o preenchimento do planejamento?')}>{busy?'Processando…':'Validar TI e liberar planejamento'}</button>}{item.status==='awaiting_dates'&&<><span className="kanban-legacy-note">Datas registradas. Abra o cartão para revisar o planejamento.</span><button type="button" onClick={()=>onSelect(item.id)}>Abrir planejamento</button></>}{item.status==='planning'&&<button type="button" onClick={()=>onSelect(item.id)}>Abrir planejamento</button>}{item.status==='confirmed'&&<button type="button" onClick={()=>onSelect(item.id)}>Consultar planejamento</button>}</div></article>}):<div className="kanban-empty">Nenhum inventário nesta etapa.</div>}</div></div>})}</div></section>}
function AuditSummary({audit,onNavigate}){return <div className={`audit-summary ${audit.approvalReady?'is-ready':'has-pending'}`}><div className="audit-summary-head"><div><h3>{audit.approvalReady?'Planejamento pronto para aprovação':'Pendências por bloco'}</h3><p>{audit.approvalReady?'Todos os blocos aplicáveis e checklists estão completos.':`${audit.approvalIssues.length} pendência(s) precisam de atenção antes da aprovação.`}</p></div>{audit.validationErrors.length>0&&<span className="audit-warning">{audit.validationErrors.length} inconsistência(s)</span>}</div>{audit.pendingBlocks.length>0&&<div className="audit-blocks">{audit.pendingBlocks.map(block=><article key={block.key} className="audit-block-link" role="button" tabIndex={0} onClick={()=>onNavigate?.(block.key)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();onNavigate?.(block.key)}}}><div><b>{block.label}</b><span>{block.percentage}%</span></div><ul>{block.issues.slice(0,4).map(issue=><li key={issue}>{issue}</li>)}</ul>{block.issues.length>4&&<small>+ {block.issues.length-4} pendência(s) neste bloco</small>}<em>Ir para o bloco</em></article>)}</div>}</div>}
function Block({blockKey,title,subtitle,children,completed=false,onToggle,readOnly=false}){const [expanded,setExpanded]=useState(!completed);useEffect(()=>{if(!completed)setExpanded(true)},[completed]);const handleToggle=()=>{if(readOnly){setExpanded(current=>!current);return}onToggle?.();setExpanded(!completed)};return <section id={blockKey} className={`card plan-block ${completed?'is-completed':''} ${readOnly?'is-readonly':''}`}><div className="plan-block-header"><div className="plan-block-title"><h2>{completed&&<span className="block-check" aria-hidden="true">✓</span>}{title}</h2>{(!completed||expanded)&&subtitle&&<p>{subtitle}</p>}</div><div className="plan-block-controls">{completed&&<span className="block-complete-label">Concluído</span>}{!readOnly&&<button type="button" className="block-complete-button" onClick={handleToggle}>{completed?'Reabrir bloco':'Concluir bloco'}</button>}</div></div>{completed&&!expanded?<button type="button" className="block-collapsed-summary" onClick={()=>setExpanded(true)}><span>✓ Bloco concluído</span><span>Expandir para consultar</span></button>:<fieldset className="plan-block-body" disabled={readOnly}>{children}</fieldset>}</section>}
