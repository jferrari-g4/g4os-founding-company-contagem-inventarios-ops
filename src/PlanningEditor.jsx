...
const planningStatus={received:'Recebida',awaiting_dimensioning:'Dimensionamento',awaiting_ti:'Aguardando TI',awaiting_dates:'Datas enviadas',planning:'Planejamento',confirmed:'Confirmada'};
...
export default function PlanningEditor({projects,user,canApprove=false,catalogs={},onSaved}){
  ...
  return <section className="planning-shell">
    <PlanningKanban projects={projects} onSelect={selectProject}/>
...
function PlanningKanban({projects,onSelect}){const active=projects.filter(item=>item.status!=='cancelled');return <section className="planning-kanban"><div className="kanban-heading"><div><span className="eyebrow">Controle operacional</span><h2>Inventários em movimento</h2><p>Acompanhe a passagem de cada operação até o planejamento aprovado. Clique em um cartão para abrir os detalhes.</p></div><span className="kanban-total">{active.length} {active.length===1?'inventário':'inventários'}</span></div><div className="kanban-board">{planningColumns.map(column=>{const items=active.filter(item=>column.statuses.includes(item.status));return <div className={`kanban-column kanban-column-${column.key}`} key={column.key}><div className="kanban-column-head"><div><span className="kanban-dot"/><b>{column.label}</b></div><span>{items.length}</span></div><div className="kanban-cards">{items.length?items.map(item=>{const content=item.planning?.content||{};const audit=auditPlanning(content);const store=content.store||{};const dates=store.inventoryStart&&store.inventoryEnd?`${formatDate(store.inventoryStart)} – ${formatDate(store.inventoryEnd)}`:'Período não definido';const pending=item.planning?audit.approvalIssues.length:null;return <article className="kanban-card" key={item.id} onClick={()=>onSelect(item.id)}><div className="kanban-card-top"><span className="kanban-reference">{item.reference||'Sem referência'}</span><span className={`kanban-status kanban-status-${item.status}`}>{planningStatus[item.status]||item.status}</span></div><h3>{item.company_name||'Cliente sem cadastro'}</h3><p className="kanban-meta">{store.unit||item.inventory_type||'Unidade não definida'}</p><div className="kanban-card-info"><span><b>Período</b>{dates}</span><span><b>Dias</b>{content.days?.length||0}</span></div><div className="kanban-progress-line"><i style={{width:`${audit.percentage}%`}}/></div><div className="kanban-card-bottom"><span>{item.planning?`${audit.percentage}% preenchido`:'Sem planejamento iniciado'}</span>{pending===null?<span className="kanban-pending neutral">Ação necessária</span>:pending>0?<span className="kanban-pending">{pending} pendência(s)</span>:<span className="kanban-pending ready">Pronto</span>}</div><button className="kanban-open" onClick={event=>{event.stopPropagation();onSelect(item.id)}}>Abrir planejamento</button></article>;}):<div className="kanban-empty">Nenhum inventário nesta etapa.</div>}</div></div>})}</div></section>}
